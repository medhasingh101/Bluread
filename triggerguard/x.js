  ...
  (function () {
    ...
    function getVideoContainers() {
      ...
      videos.forEach((video) => {
        ...
        let container;
        if (isInstagram) {
          // Tightest possible container to avoid wrapping the whole post/article
          container = video.closest('div[data-pressable-container="true"]') ||
            video.parentElement;
        } else {
    ...
    function getContextText(container) {
      ...
      if (isInstagram) {
        // Find the specific caption container rather than grabbing all text which includes comments
        const article = container.closest('article') || document.querySelector('main');
        if (!article) return '';
        // On Instagram, the overarching span inside the h1 (post author + caption) is usually the caption
        // We avoid deep querying all spans as it snags comments and flags the video falsely
        const captionElement = article.querySelector('h1 + div span, div[data-testid="post-comment-root"] span');
        return captionElement ? (captionElement.textContent || '').trim().slice(0, 500) : '';
      }
      return '';
    }
    ...
    `;
      ...
      modal.querySelector('.tg-btn-reveal').addEventListener('click', () => {
        ...
        wrapper.querySelector('.tg-label--video')?.remove();
        modal.remove();
      });
    }
